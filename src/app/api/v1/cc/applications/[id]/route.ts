...
async function validateTransition(id: string, data: Prisma.CcApplicationUpdateInput) {
  ...
  const nextStatus = String(data.admissionStatus ?? app.admissionStatus);
  ...
  if (SUBMITTED_OR_LATER.has(nextStatus) && app.profile.conflictStatus === 'red' && !app.profile.riskFlagCleared) {
...
export async function PUT(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const validation = await validateTransition(id, data);
