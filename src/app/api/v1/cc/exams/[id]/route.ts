...
async function guardBranch(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const exam = await prisma.ccExam.findUnique({
    where: { id },
    select: { profile: { select: { branchId: true } } },
  });
  if (!exam) return { response: errorResponse('NOT_FOUND', 'Запись не найдена', 404) };
  if (!(await canAccessCcProfileBranch(auth.session.user, exam.profile.branchId))) {
